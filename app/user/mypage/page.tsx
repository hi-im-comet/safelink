    ...
    <section className="overflow-hidden rounded-xl3 bg-white ring-1 ring-ink/8">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between px-4 py-3.5 text-left">
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="shrink-0 rounded-full bg-mint-soft px-2 py-0.5 text-[0.7rem] font-bold text-pine ring-1 ring-green/10">
            {badge}
          </span>
        </span>
        <Chevron open={open} />
      ...
      {open && (
        <div className="flex flex-col gap-3.5 border-t border-ink/5 p-4">
          {note && <p className="rounded-xl bg-mint-soft/50 px-3 py-2 text-xs font-medium text-forest/65">{note}</p>}
          {children}
        </div>
      )}
    </section>
...
장애 여부: ${values.disability === "있음" ? values.disabilityType || "있음" : "없음"}
이동 불편 여부: ${values.mobility || "-"}
복용약: ${values.medications || (values.meds === "있음" ? "있음" : "없음")}
자주 가는 병원: ${values.hospital || "-"}
보호자 연락처: ${values.guardianPhone || "-"}
동거인 연락처: ${values.cohabitPhone || "-"}`;
  ...
  const riskNote = "이 정보는 폭염 위험도와 도움 요청 우선순위 판단에 반영됩니다.";
  ...
  return (
    <div className="stagger flex flex-col gap-4">
      ...
      {/* 기본 정보 */}
      <Section title="기본 정보" badge="선택" defaultOpen>
        {Text("name", "이름", "홍길동")}
      ...
      {/* 건강 정보 */}
      <Section title="건강 정보" badge="선택" note={riskNote}>
      ...
      {/* 비상 연락 정보 */}
      <Section title="비상 연락 정보" badge="권장" note={riskNote}>
      ...
      {/* 생활·돌봄 정보 */}
      <Section title="생활 · 돌봄 정보" badge="권장" note={riskNote}>
      ...
      <section className="rounded-xl3 bg-ember-soft/60 p-5 ring-1 ring-ember/20">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-white/70 px-2.5 py-0.5 text-xs font-semibold text-ember-ink">자동 정리</span>
        </div>
        <p className="mt-1 text-sm text-ember-ink/80">응급 상황에서 직접 전화하거나 복사해 전달할 수 있도록 필요한 정보를 정리합니다.</p>
        <p className="mt-3 text-xs font-bold uppercase tracking-wide text-ember-ink/60">복사 전 미리보기</p>
        ...
        <div className="mt-3 flex items-center justify-end gap-2">
          {copied && (
            <span className="animate-fade-up rounded-full bg-white/80 px-3 py-1.5 text-xs font-bold text-ember-ink ring-1 ring-ember/15">
              비상정보가 복사되었습니다.
            </span>
          )}
          <CopyButton text={emergencyText} label="비상정보 복사" onCopied={showCopiedToast} />
        </div>
      </section>
      ...
      <ul className="flex flex-col gap-1 text-center text-xs leading-relaxed text-forest/45">
        <li>개인정보는 도움 요청과 위험도 판단에만 사용됩니다.</li>
        <li>이름·연락처·상세 건강정보는 LLM에 전달하지 않습니다.</li>
